...
const getSymbolConfig = (symbol: string) => (CONFIG as any).SYMBOLS[symbol];
...
export class WebDashboardServer {
  ...
  private handlePerformanceApi(req: http.IncomingMessage, res: http.ServerResponse) {
   ...
   private async handleAIFeedback(req: http.IncomingMessage, res: http.ServerResponse) {
     try {
    ...
    private async handleParameterOverride(req: http.IncomingMessage, res: http.ServerResponse) {
      try {
        const body = await this.readBody(req);
        ...
        if (obiThreshold !== undefined) ((CONFIG.SYMBOLS as any)[targetSymbol]?.obiThreshold && (CONFIG.SYMBOLS as any)[targetSymbol].obiThreshold !== undefined) && (CONFIG.SYMBOLS as any)[targetSymbol].obiThreshold !== undefined && (obiThreshold !== undefined) && ((CONFIG.SYMBOLS as any)[targetSymbol].obiThreshold = obiThreshold);
        if (zScoreThreshold !== undefined) ((CONFIG.SYMBOLS as any)[targetSymbol]?.zScoreThreshold && (CONFIG.SYMBOLS as any)[targetSymbol].zScoreThreshold !== undefined) && (CONFIG.SYMBOLS as any)[targetSymbol].zScoreThreshold !== undefined && (zScoreThreshold !== undefined) && ((CONFIG.SYMBOLS as any)[targetSymbol].zScoreThreshold = zScoreThreshold);
        if (takeProfitPct !== undefined) ((CONFIG.SYMBOLS as any)[targetSymbol]?.takeProfitPct && (CONFIG.SYMBOLS as any)[targetSymbol].takeProfitPct !== undefined) && (CONFIG.SYMBOLS as any)[targetSymbol].takeProfitPct !== undefined && (takeProfitPct !== undefined) && ((CONFIG.SYMBOLS as any)[targetSymbol].takeProfitPct = takeProfitPct);
        if (stopLossPct !== undefined) ((CONFIG.SYMBOLS as any)[targetSymbol]?.stopLossPct && (CONFIG.SYMBOLS as any)[targetSymbol].stopLossPct !== undefined) && (CONFIG.SYMBOLS as any)[targetSymbol].stopLossPct !== undefined && (stopLossPct !== undefined) && ((CONFIG.SYMBOLS as any)[targetSymbol].stopLossPct = stopLossPct);
    ...
    private handleParameterOverrideGet(req: http.IncomingMessage, res: http.ServerResponse) {
      try {
   ...
   private async handleIndicatorConfig(req: http.IncomingMessage, res: http.ServerResponse) {
     try {
       const indicators = [
   ...
   private async handleIndicatorRecommendations(req: http.IncomingMessage, res: http.ServerResponse) {
     try {
       const data = this.performanceDataProvider ? this.performanceDataProvider() : null;
       ...
       const indicatorStats: Record<string, { wins: number; losses: number; total: number; winRate: number }> = {};
       ...
       const analyzeReason = (trades: any[], keyword: string) => {
       ...
       if (obi) {
         if (obi.winRate >= 60) recommendations.push({ type: 'positive', text: `OBI threshold efektif: WR ${obi.winRate.toFixed(0)}% pada ${obi.count} trade.`, confidence: 'MEDIUM' });
         else if (obi.winRate < 40) recommendations.push({ type: 'negative', text: `OBI threshold kurang efektif: WR ${obi.winRate.toFixed(0)}%. Pertimbangkan turunkan threshold.`, confidence: 'MEDIUM' });
       }

       const vwap = analyzeReason(allTrades, 'vwap');
       if (vwap && vwap.winRate >= 60) recommendations.push({ type: 'positive', text: `VWAP filter membantu: WR ${vwap.winRate.toFixed(0)}% pada ${vwap.count} trade.`, confidence: 'MEDIUM' });

       const fib = analyzeReason(allTrades, 'fib');
       if (fib && fib.winRate < 40) recommendations.push({ type: 'negative', text: `Fibonacci levels kurang relevan: WR ${fib.winRate.toFixed(0)}% pada ${fib.count} trade.`, confidence: 'MEDIUM' });

       const obiTrend = analyzeReason(allTrades, 'obi trend');
       if (obiTrend && obiTrend.winRate >= 60) recommendations.push({ type: 'positive', text: `OBI Trend momentum efektif: WR ${obiTrend.winRate.toFixed(0)}% pada ${obiTrend.count} trade.`, confidence: 'MEDIUM' });
