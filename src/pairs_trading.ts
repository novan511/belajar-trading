...
import { PairsSignal } from './types.js';

interface PairState {
  symbolA: string;
  symbolB: string;
  priceHistoryA: number[];
  priceHistoryB: number[];
  spreadValues: number[];
  correlation: number;
  hedgeRatio: number;
  spreadMean: number;
...
export class PairsTrader {
  ...
  private cooldownPeriodMs = 300000; // 5 min cooldown between signals
   ...
   */
  public updatePrice(symbol: string, midPrice: number) {
   ...
   */
  private updateSpread(state: PairState) {
    const { priceHistoryA, priceHistoryB } = state;
    if (priceHistoryA.length < 20 || priceHistoryB.length < 20) return;
   ...
   */
  public checkSignal(symbolA: string, symbolB: string): PairsSignal | null {
    const key = this.getPairKey(symbolA, symbolB);
    const state = this.pairs.get(key);
    if (!state) return null;

    // Cooldown check
    if (Date.now() - state.lastSignalTime < this.cooldownPeriodMs) return null;

    // Need enough data
    if (state.spreadValues.length < 20 || state.spreadStd <= 0) return null;
   ...
   */
  public shouldClose(symbolA: string, symbolB: string): boolean {
    const key = this.getPairKey(symbolA, symbolB);
    const state = this.pairs.get(key);
    if (!state || state.spreadValues.length < 10) return false;
   ...
   */
  public getSpreadInfo(symbolA: string, symbolB: string) {
    const key = this.getPairKey(symbolA, symbolB);
    const state = this.pairs.get(key);
    if (!state) return null;
