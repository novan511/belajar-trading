...
import { OrderBook, OrderFlowState, VWAPData, VolumeProfile, LiquiditySweepSignal, Side } from './types.js';

export class MarketMicrostructure {
  // OBI History for trend detection
  private obiHistory: Map<string, number[]> = new Map();
  
  // Tick-level trade flow tracking
  private bidVolumes: Map<string, number[]> = new Map();
  private askVolumes: Map<string, number[]> = new Map();
  
  // CVD tracking
  private cvdValues: Map<string, number> = new Map();
  private cvdHistory: Map<string, number[]> = new Map();
  private lastMidPrices: Map<string, number[]> = new Map();

  // Liquidity sweep detection
  private priceHistory: Map<string, { high: number; low: number }[]> = new Map();

  // VWACalculation
  private vwapCumulativePrice: Map<string, number> = new Map();
  private vwapCumulativeVolume: Map<string, number> = new Map();
  private vwapData: Map<string, VWAPData> = new Map();
   ...
   */
  public getOBITrend(symbol: string): 'ACCUMULATING' | 'DISTRIBUTING' | 'NEUTRAL' {
    const hist = this.obiHistory.get(symbol);
    if (!hist || hist.length < 5) return 'NEUTRAL';
    ...
    if (recent > older + 0.05) return 'ACCUMULATING';
    if (recent < older - 0.05) return 'DISTRIBUTING';
    return 'NEUTRAL';
  }
   ...
   */
  public getOFI(symbol: string): number {
    const bidVols = this.bidVolumes.get(symbol);
    const askVols = this.askVolumes.get(symbol);
    if (!bidVols || !askVols || bidVols.length < 3) return 0;
   ...
   */
  public checkCVDDivergence(symbol: string): 'BULLISH' | 'BEARISH' | null {
    const cvdHist = this.cvdHistory.get(symbol);
    const priceHist = this.lastMidPrices.get(symbol);

    if (!cvdHist || !priceHist || cvdHist.length < 10) return null;
   ...
   */
  public detectLiquiditySweep(symbol: string, bestBid: number, bestAsk: number): LiquiditySweepSignal | null {
    const pxHist = this.priceHistory.get(symbol);
    if (!pxHist || pxHist.length < 10) return null;
   ...
   */
  public getVWAP(symbol: string): VWAPData | null {
   ...
   */
  public getVolumeProfile(symbol: string): VolumeProfile | null {
    const bidVols = this.bidVolumes.get(symbol);
    const askVols = this.askVolumes.get(symbol);
    if (!bidVols || !askVols || bidVols.length < 10) return null;

    // Use OBI to determine where volume is concentrated
    const obis = this.obiHistory.get(symbol) || [];
    if (obis.length < 10) return null;
   ...
   */
  public getOrderFlowState(symbol: string): OrderFlowState {
    const bidVols = this.bidVolumes.get(symbol) || [];
    const askVols = this.askVolumes.get(symbol) || [];
