...
import { ExecutionStats, TradeRecord, TradingSession } from './types.js';
...
export class RiskManager {
  ...
  private symbolATR: Map<string, number> = new Map();
  private symbolATRHistory: Map<string, number[]> = new Map();
  private readonly atrPeriod = 14;
  
  // Equity curve & metrics
  private equityCurve: number[] = [];
  private allTradeRecords: TradeRecord[] = [];
  private consecutiveLosses: number = 0;
  private maxConsecutiveLosses: number = 0;
  
  // Track all daily balances for metric calculations
  private allBalances: number[] = [];
   ...
   */
  calculateRiskMetrics(trades: TradeRecord[]): RiskMetrics {
   ...
   */
  private calculateRiskOfRuin(winRate: number, avgWinLossRatio: number): number {
    if (winRate <= 0 || winRate >= 1) return 0;
    if (avgWinLossRatio <= 0) return 1;
   ...
   */
  getEquityCurve(): { time: number; equity: number }[] {
   ...
   */
  getLatestMetrics(trades: TradeRecord[]): RiskMetrics {
  ...
  public updateATR(symbol: string, atr: number) {
    if (atr <= 0 || isNaN(atr) || !isFinite(atr)) return;
    this.symbolATR.set(symbol, atr);
    const history = this.symbolATRHistory.get(symbol) || [];
    history.push(atr);
    if (history.length > this.atrPeriod * 2) history.shift();
