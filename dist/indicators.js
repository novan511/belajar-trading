 ...
 */
export function calculateFVGs(candles) {
    const fvgs = [];
    if (candles.length < 3)
        return fvgs;
 ...
 */
export function calculateSRLevels(candles, period = 5, tolerancePct = 0.005) {
    const rawLevels = [];
 ...
 */
export function calculatePOC(candles, binsCount = 20) {
    if (candles.length === 0)
        return 0;
