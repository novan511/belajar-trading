...
export class SmartOrderRouter {
    MAX_SLIPPAGE_PCT = 0.002; // 0.2% max slippage per slice
    MIN_SLICE_SIZE_PCT = 0.1; // Min 10% dari total order
    MAX_SLICE_SIZE_PCT = 0.5; // Max 50% dari total order
    TWAP_SPREAD_MS = 1000; // 1 detik antar slice
    PRICE_IMPROVEMENT_THRESHOLD = 0.0003; // 0.03% improvement = better price
    lastOrderTimes = new Map();
    marketMicro; // MarketMicrostructure reference
    constructor(marketMicro) {
     ...
     */
    createExecutionPlan(symbol, side, totalQty, book, urgency = 'NORMAL') {
        ...
        const sliceQty = Math.min(totalQty / sliceCount, maxSliceSize, coinConfig.lotSize * 100);
        const slices = [];
     ...
     */
    async executePlannedOrder(symbol, side, totalQty, book, executeFn, urgency = 'NORMAL') {
        const coinConfig = Object.values(CONFIG.SYMBOLS).find(s => s.name === symbol);
        if (!coinConfig)
            throw new Error(`Unknown symbol: ${symbol}`);
     ...
     */
    async executeMarketOrder(symbol, side, qty, executeFn, book) {
     ...
     */
    isGoodEntryCondition(symbol, side, book) {
        const bestBid = book.bids[0][0];
        const bestAsk = book.asks[0][0];
